  ...
  return (
    ...
    >
      <Link to={`/tour/${tour.id}`} className="block group relative h-full">
        <div className="relative h-64 md:h-80 w-full rounded-2xl overflow-hidden shadow-warm-lg transition-transform duration-300 group-hover:-translate-y-1 group-focus:ring-4 ring-museum-moss/50 bg-museum-walnut/20">
          {/* Background Image */}
          {tour.image && !imgError && (
            <img
              src={tour.image}
          ...
          <div className="absolute inset-0 p-6 flex flex-col justify-end text-museum-cream">
            <div className="transform transition-transform duration-300 group-hover:-translate-y-2">
              <div className="flex items-center gap-3 text-xs font-medium uppercase tracking-wider mb-2 text-museum-cream/80">
                ...
                <span className="flex items-center gap-1 bg-museum-walnut/30 backdrop-blur-md px-2 py-1 rounded-md border border-museum-cream/10">
                  <MapPin size={12} /> {tour.stopIds.length} {ui.stops}
                </span>
              </div>

              <h3 className="text-2xl font-bold mb-2 text-shadow-sm leading-tight">{title}</h3>
              <p className="text-sm text-museum-cream/90 line-clamp-2 mb-4 font-light">{description}</p>
